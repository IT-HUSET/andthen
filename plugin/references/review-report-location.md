...
3. **Current feature directory** – when no spec directory resolves, infer the active feature from the in-progress story rows. When a `plan.json` governs current work (it governs while it has undone – not `done`/`skipped` – stories; discoverable via the Project Document Index `Specs & Plans` location or the FIS paths the State references), derive rows from `stories[]` with `status: in-progress`, excluding `fis: null`, and resolve each canonical basename pointer beside that plan before using it as FIS. This is deliberately narrower than read-state's in-progress-or-claimed union – a claimed-but-unstarted story is not the *current* feature. Otherwise read FIS paths from the stored `STATE.md` **Active Stories** table (per the Project Document Index `State` row, default `docs/STATE.md`). Destination is `dirname(resolved FIS)` – the feature directory holding the active FIS:
   ...
   - **Skip** when neither source resolves (no governing `plan.json`, and `STATE.md` missing or without an **Active Stories** section), or no candidate rows remain (`fis: null` rows are excluded per-row above – they never skip the tier on their own)
