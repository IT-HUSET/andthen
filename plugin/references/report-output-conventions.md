...
- **Typed artifact**: Follow `${CLAUDE_PLUGIN_ROOT}/references/github-artifact-roundtrip.md`. Use the report suffix as the default `artifact_type` (for example `review`, `gap-review`, or `code-review`) unless the calling skill requires a narrower type.
