...
**Body size**: when the assembled body exceeds the 65,536-char limit, do not truncate. Create the issue with the body sans the largest extractable section (typically `## Binding Constraints` for plan bodies when a PRD has many verbatim spans, or per-story sections — preserve the parser-friendly H2 anchor with a single-line stub like `_See follow-up comment for full content._`), capture the new issue number, then post the omitted section as a follow-up comment on that same issue via Pattern B's mechanics (`gh issue comment <N> --body-file <section-path>`, splitting further if a single section still exceeds the limit). Surface this as a multi-step run in the host's report so the user sees the supplemental comments.
