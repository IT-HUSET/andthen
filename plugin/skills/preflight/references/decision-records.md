...
Emitted as a bare line at line start, one resolved token, once, beside any verdict block. Consumer regex: `^Preflight: (READY|DEFERRED|BLOCKED)$`.
...
- **DEFERRED** – converged, but one or more remaining decisions are signed-off deferrals; none still `open`. (A bundle is DEFERRED only when every story is clear-or-deferred.)
