...
ARGUMENTS: $ARGUMENTS (strip flag tokens `--auto` and `--limit N` before interpreting the remainder as specific issue number(s) or a tracker query; empty remainder → triage the untriaged backlog)
...
- **AI attribution** – every comment triage posts opens with the line `> _Generated by AI during triage._` so a human scanning the tracker can tell machine-authored notes from teammates' at a glance.
...
7. **Apply** – on the ratified outcome, in order:
   ...
   - **Rationale comment** – `comment` (attribution line first) with the rationale and any pointer, for every outcome.
   - **Agent brief** (`ready-for-agent` only) – `edit body` to append a clearly-delimited `## Agent Brief` section (authored per `references/agent-brief.md`, attribution line first inside it) to the issue body; on re-triage, replace that section in place so it stays idempotent. The brief travels in the body – it is the handoff payload a downstream executor reads.
...
- Apply only **safe transitions** – recommend `needs-info` or `ready-for-human` as a comment (attribution line first) and label accordingly.
