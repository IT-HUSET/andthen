...
`edit body` appends the section below to the issue body – delimited by its `## Agent Brief` heading, attribution line first inside it – and replaces it in place on re-triage so it stays idempotent:
