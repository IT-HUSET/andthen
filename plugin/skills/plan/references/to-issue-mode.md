...
Synthesize per the **single-issue shape** in [`plan-issue-shape.md`](${CLAUDE_PLUGIN_ROOT}/references/plan-issue-shape.md): plan summary → optional `## Shared Decisions` → optional `## Binding Constraints` → `## Story Catalog` table → one `### Story S0N: <name>` per story → `Refs #<input-issue-N>` footer line when an input issue was supplied.
