...
Capture each story's `id`, `status`, `fis`, `owner`, the content-defining fields per the **Preservation predicate** (see *Writability rules* in *The Plan Schema*), and `executionNotes` into a preservation map. Before regeneration, normalize a legacy relative v1 FIS pointer through `andthen:ops update-plan-fis` only when its project-root-relative realpath is the exact canonical sibling and its regular-file, non-symlink, and Plan/Story provenance checks pass; re-read the plan into the map. Discard legacy `metadata` fields (e.g. `immutableDigest`) – not in the current schema. Continue Step 2 and proceed through 3–4 as a fresh generation. After Step 4's reassembly:
...
- **Restore `status`, `fis`, and `owner`** from the normalized preservation map per the **Preservation predicate**. Stories failing compatibility or any predicate clause reset to `pending` / `null` / `null`.
