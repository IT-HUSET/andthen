...
- Apply `plan-schema.md` PSCH-53 to every populated `FIS` cell: a compatible file normalizes to its canonical basename and preserves status; any invalid pointer resets to `fis: null`, `status: "pending"`. FIS-unset rows use the same reset.
