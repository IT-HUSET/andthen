...
- **Build the execution plan**: parse the `## Story Catalog` table for IDs, dependencies, wave assignments. Validate the catalog `Dependencies` cells before scheduling: values must be `-` or comma-separated Story IDs from the same catalog. Stop on prose or unknown IDs with `BLOCKED: invalid dependency in <story-id>: "<value>" — use Story IDs in the catalog and put milestone prose in Dependency Graph or issue body notes.` In granular shape, also map each catalog `ID` to its story-issue `#<N>` from the `## Story Issues` section. Story-issue `Depends on #<N>` notes are optional navigation, not scheduler input; if present, accept them only after mapping issue numbers back to catalog Story IDs.
