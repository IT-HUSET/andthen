...
- **Re-executing a composite FIS that was already implemented** — when multiple stories share the same FIS path, exec-spec should only run once. Check the executed-FIS set before each story's pipeline
...
**Shared-FIS Dedup** (check before executing each story):
If this story's FIS path has already been executed by a previously completed story (composite FIS — multiple stories share the same FIS file), **skip exec-spec and review** for this story. The implementation already exists. Still run the **Plan Acceptance Gate** (Step 2c) to verify this story's specific acceptance criteria are met, then mark `Done`. Track executed FIS paths in a set to avoid re-execution.
...
After each story completes its required stages for the selected `REVIEW_MODE`:
