...
Write to `.agent_temp/walkthrough/<slug>-walkthrough-<YYYY-MM-DD>.md` (repo root; `<slug>` = `pr-<N>` or the kebab-cased branch name). Follow this template exactly – headings and table/fence shapes are the contract the `andthen:visualize` skill's changeset renderer parses:
