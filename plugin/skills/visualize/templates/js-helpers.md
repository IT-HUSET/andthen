...
**JSON script discipline:** emit the detail dictionary via `JSON.stringify` in inert `application/json`, using the render-shell Safe Output Boundary; multiline body remains `\n` data. **Name mismatch:** absent `dataByKey` returns early and preserves the panel. Emit static `<!-- module-map: node detail missing -->` per unpaired node; artifact names never enter comments. **Security:** assign title/meta/body with `textContent` only (Safe Output Boundary – no `innerHTML`) and preserve line breaks in CSS.
