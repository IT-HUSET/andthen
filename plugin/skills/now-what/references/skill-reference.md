...
**Use when:** structural questions, comparing options, mapping a domain end-to-end, or before committing to a decomposition. `--visual` delegates structured reports (`review`, `trade-off`, `strategic-design`, `fitness`, `decompose`, `event-storming`, ADR) to the `andthen:visualize` skill for browser review; pure `advise` is text-only. `strategic-design` registers the accepted context map into the `Context Map` document (user-gated). **Typical next step:** back to `andthen:now-what` once the design question is resolved.
