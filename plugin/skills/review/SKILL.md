...
**Per-lens dispatch detail**: code-lens specialists and thin security awareness per `references/lens-code.md`; security-lens applicability gate, checklists, trust-boundary, scanners, and Critic per `references/lens-security.md`; council orchestration (within-lens councils for `code`/`security`, cross-lens pass on 2+-lens chains feeding `## Cross-Lens Synthesis`) per `references/council-mode.md`.
...
**Reconciliation-ledger match** (when a ledger was loaded in Step 1). Compute each finding's stable ID and route by its matched entry status/class per [`reconciliation-ledger.md`](${CLAUDE_PLUGIN_ROOT}/references/reconciliation-ledger.md) (§ Stable finding ID, § Match-and-route rules, § Recurrence and escalation): OPEN `code-defect` keeps feeding the verdict; OPEN `spec-stale` / `design-changed` / `ambiguous-intent` and `RECONCILE REQUIRED` matches route `Routing: Note` tied to the entry; CLOSED / WITHDRAWN stay suppressed unless this run cites evidence refuting the recorded falsifier (re-open in place via the `andthen:ops` skill `update-ledger add`). On an unreconciled OPEN `spec-stale` / `design-changed` re-surfacing, call `update-ledger bump-recurrence`; recurrence 2 → `RECONCILE REQUIRED`, clearable only via `update-fis design-change` + ADR.
...
**Auto-Remediation signal (loop control).** Emit one additive line `Auto-Remediation: PENDING | STALLED | CLEAR` beside (never inside) the `## Verdict` block (`PENDING` / `STALLED` / `CLEAR` definitions in `references/review-verdict.md` § Loop Convergence Signals). Under `--council`, compute it from the final post-synthesis routed findings only, so advisory perspectives that do not survive synthesis never inflate it.
