...
- `--council` → multi-perspective review with debate (structured findings, Findings Filter, synthesis). Scales with the chain shape – within-lens specialist councils (5-7 reviewers) augment `code` and `security`; chains add a cross-lens Critic + Devil's Advocate + Synthesis Challenger pass on top:
  ...
  | Single `doc` or `gap` | **Reject up-front** (see the INSTRUCTIONS reject list for the verbatim `BLOCKED:` string). |
...
- **Routing gate** – classify every accepted finding (`Class:` + `Routing: Fix|Note`) before the report; `--fix` auto-applies Fix only. Safe document/workflow-artifact defects can route to Fix; unresolved intent/product/design/reconciliation decisions stay Note. Mechanism, class enum, and recall rationale in Step 5a.
...
- **Calibration-first / anti-leniency**: load `${CLAUDE_PLUGIN_ROOT}/references/review-calibration.md` (universal, § Anti-Leniency Protocol) plus each lens's calibration (cited by each lens reference) before categorising findings; record findings at find time – severity and dismissal belong to calibration and the Findings Filter, not to ad-hoc rationalization. The Critic sub-lens also loads `${CLAUDE_PLUGIN_ROOT}/references/critic-calibration.md`; the Findings Filter uses `${CLAUDE_PLUGIN_ROOT}/references/findings-filter-templates.md` after findings are collected.
...
- **Routing every accepted finding into Fix** – find-passes favor recall, so most reviews accept more than warrant auto-application; collapsing the buckets is how `--fix` turns into chain-drift. Fix-bucket criteria and recall rationale: Step 5a.
...
**Reconciliation-ledger load.** When the review covers code against a governing FIS (code/gap lenses), read that FIS's adjacent ledger (`{fis-without-ext}.reconciliation-ledger.md`). It is the cross-run record of already-tracked spec-vs-code drift; Step 5a matches findings against it so known reconciliation work is not re-raised as a fresh blocker and a withdrawn finding cannot silently re-raise. A **doc-lens-only review, or any review with no governing FIS, loads no ledger** – the ledger tracks the code↔FIS boundary, and doc-lens findings are classified but never ledgered. No ledger → record `Reconciliation Ledger: none` and skip the Step 5a match step. Schema, adjacency, and match/recurrence rules: [`reconciliation-ledger.md`](${CLAUDE_PLUGIN_ROOT}/references/reconciliation-ledger.md).
...
**Reconciliation-ledger match** (when a ledger was loaded in Step 1). For each accepted finding, compute its **stable ID** = `{relative-path}:{class}:{normalized-title-slug}` and match against ledger entries, **keying primarily on `{relative-path}:{class}`** (the normalized-title-slug only disambiguates multiple entries sharing the same path and class) per [`reconciliation-ledger.md`](${CLAUDE_PLUGIN_ROOT}/references/reconciliation-ledger.md). Matching is exact-string – no hashing, semantic similarity, or version vector clock. Route by the matched entry's status:

A finding matches only an entry of its **own class** (`{class}` is part of the matching key), so the OPEN-match branch is class-aware:

- **OPEN match, `spec-stale` / `design-changed`** → already-tracked reconciliation work, **not** a fresh blocker: record it as `Routing: Note` tied to the existing entry. It does **not** count toward the functional verdict's FAIL conditions. If still unreconciled, call the `andthen:ops` skill `update-ledger bump-recurrence <ledger-path> <stable-id>` (recurrence escalation below).
- **OPEN match, `code-defect`** → a known-but-unfixed bug: keep class `code-defect` and **continue feeding the verdict** (it stays a real blocker until fixed – do **not** demote it to Note, which would hide an unfixed defect). No recurrence escalation. It is *not* "new" for the CONVERGED criterion below.
- **OPEN match, `ambiguous-intent`** → decision-blocked: record as `Routing: Note` tied to the entry; no recurrence escalation; does not feed the verdict; stays OPEN until the missing decision is supplied.
- **RECONCILE REQUIRED match** → existing blocking reconciliation work: record as `Routing: Note` tied to the entry, do not bump recurrence, do not duplicate, and do not feed reconciliation-class findings into the verdict dimensions. The completion-presentation gate remains the blocker until the sanctioned design-change + ADR reconciliation closes the entry.
- **CLOSED / WITHDRAWN match** → keep the finding in the report's withdrawn/suppressed section; do **not** promote it to a blocker. **Exception**: if this run supplies new evidence that explicitly refutes the recorded falsifier, re-open the entry **in place** via the `andthen:ops` skill `update-ledger add <ledger-path> <stable-id> <class> <stale-targets> <source-run> <refuting-evidence>` – `add`'s terminal-match branch transitions the existing entry back to OPEN preserving its identity and prior falsifier (it does not duplicate). Surface the finding with that evidence cited. Absent a concrete refutation, a withdrawn finding stays suppressed – this is what stops a finding withdrawn on a *false* falsifier from silently re-raising as a fresh blocker next run.
- **No match** → a genuinely new finding; route normally below.

**Recurrence escalation** (`spec-stale`/`design-changed` only). On an OPEN entry re-surfaced while still unreconciled, the `update-ledger bump-recurrence` above increments the entry's recurrence; when the count reaches **2** the entry transitions to `RECONCILE REQUIRED` (a blocking status). `RECONCILE REQUIRED` is clearable **only** via the sanctioned `andthen:ops` skill `update-fis design-change` (+ ADR) → CLOSED; a bare re-run neither clears nor re-nags it. This converts an endlessly re-raised nag into a single actionable blocker.

**Class-scored verdict.** The gap-verdict's three dimensions (Functionality / Completeness / Wiring in `references/review-verdict.md`) are fed **only by `code-defect` findings**; `spec-stale` / `design-changed` / `ambiguous-intent` findings route to Note and never lower them, so the verdict reflects code-correctness alone and OPEN-matched reconciliation drift cannot drag it to FAIL. The byte-level `## Verdict` block format is unchanged.

**CONVERGED stopping criterion.** Emit a `CONVERGED` signal when this pass produced **no new `code-defect` at severity ≥ MEDIUM** – where OPEN-ledger-matched findings are *not* "new". CONVERGED is a reachable stopping criterion (stop chasing zero findings) and is emitted as an **additive line** alongside the unchanged byte-level `## Verdict` block – never inside it; the Verdict block is a parser contract and CONVERGED plus any ledger annotations are parsed separately. Definition and the Verdict-block boundary are restated in `references/review-verdict.md`.

**Fix-bucket criteria** (all must hold). Find-passes are calibrated to favor recall (per `critic-calibration.md`), so most reviews accept more findings than warrant auto-application; this gate is what keeps recall-biased findings from flowing unfiltered into `--fix` remediation:
- Confidence ≥ 75
...
- Remediation surface is deterministic and bounded:
  - implementation / security fixes require severity HIGH or CRITICAL;
  - document or workflow-artifact fixes may be any severity when the edit is mechanical or directly implied by the reviewed artifact, project rules, or source citation;
  - fixes requiring a product, design, architecture, or requirements decision are never Fix-bucket findings.
