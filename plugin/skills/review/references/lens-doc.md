...
- **FIS / spec quality** _(the primary job when a FIS or spec is reviewed before implementation)_: apply [`fis-authoring-guidelines.md`](${CLAUDE_PLUGIN_ROOT}/references/fis-authoring-guidelines.md) as acceptance gates, not a structure checklist. The title + any GWT must carry the complete observable, mechanism-distinguishing contract; inspect Proof separately to verify that articulation, never to supply missing semantics. Flag duplicated prose a durable reference already carries.
