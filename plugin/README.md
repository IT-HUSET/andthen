...
| `triage` | Investigate, diagnose, and fix build failures, config errors, runtime bugs, regressions, and test failures (`--plan-only` for a fix plan without applying; `--issue` reads a tracker-resolved issue as scope (GitHub default); `--to-issue` files the diagnosis) |
| `issue-triage` | Triage incoming issue-tracker items into a routed backlog. Per item: identify the domain concept, run a **redundancy check** (already built → comment pointing at the behavior + close, never `wontfix`), a **prior-rejection check** against the Out of Scope Registry (concept-level), and a bounded, non-destructive **claim verification**; then classify `bug`/`enhancement` and recommend one state (`needs-info`/`ready-for-agent`/`ready-for-human`/`wontfix`) for the user to ratify before any write. Applies labels per the Issue Tracker document's **Label Role Mapping**, comments with an AI-attribution line, and for `ready-for-agent` appends a durable **agent brief** to the issue body that a fresh executor can act on; a `wontfix` graduates the concept into the Out of Scope Registry. Resolves the tracker per the Issue Tracker document (GitHub is the built-in default; a `none`/absent tracker with no GitHub remote → `BLOCKED:` pointing at `init`). `--auto` holds no interview, applies only safe transitions, and reports `wontfix`/`ready-for-agent`/duplicate-close as recommendations; `--limit N` caps items. Routes ready items to `quick-implement` (small), `spec` + `exec-spec` (single feature), or `plan --issue` (multi-story). Interactive-by-Contract – sorting tracker items, distinct from debugging a live failure (that is `triage`) |
...
| `architecture` | Architecture design, review, decomposition, trade-off analysis, ADRs, fitness functions, strategic design, and event storming (modes: `review`, `decompose`, `advise`, `fitness`, `trade-off`, `strategic-design`, `event-storming`; `trade-off` updates the project's `DECISIONS.md` registry when ADR creation is accepted and routes a criterion that hinges on an empirical unknown to the `andthen:spike` skill for evidence; `strategic-design` registers the accepted context map into the `Context Map` document (user-gated, idempotent per context/pair) so later runs and other skills read one durable source; `--visual` delegates structured reports – `review`, `trade-off`, `strategic-design`, `fitness`, `decompose`, `event-storming`, and ADR – to the `andthen:visualize` skill; pure `advise` is text-only) |
...
| `clarify` | Discovery & Ideation – interactive requirements discovery at feature or product scope (`--mode product\|feature`, inferred from INPUT). Feature scope → `requirements-clarification.md`; product scope → `PRODUCT.md` (vision, personas, value props, anti-goals). Always interactive (Interactive-by-Contract; no headless mode). Checks the `Out of Scope Registry` (a concept-matching rejected direction is surfaced, not silently re-litigated) and the `Context Map` before asking; offers the `andthen:spike` skill when a load-bearing question hinges on an empirical unknown only runnable code can settle; a firmly rejected direction graduates to the `Out of Scope Registry` (distinct from the doc's own feature-level Out of Scope). Supports `--issue` for GitHub input, `--to-issue` for publishing, and `--visual` as a convenience handoff to the `andthen:visualize` skill |
| `prd` | Create a Product Requirements Document from requirements. Conversationally, load-bearing gaps are resolved by invoking the `andthen:clarify` skill inline rather than assumed (under `--auto`, assumed conservatively); produces a self-contained `prd.md` (no links to transient discovery artifacts); a firmly rejected `Scope > Out of Scope` direction graduates into the cross-feature Out of Scope Registry; runs an automatic fresh-context doc self-review via the `andthen:review` skill with `--mode doc --fix` before finishing. Supports `--issue`/`--to-issue` (resolving the tracker per the Issue Tracker document; GitHub default) and `--visual` as a convenience handoff to the `andthen:visualize` skill |
| `spec` | Generate Feature Implementation Specification from requirements; walks the `Context Map` (align FIS boundaries and integration assumptions to the registered bounded contexts; flag contradictions) among its required inputs, and sizes against the **Single-session rule** – a story plus its FIS must fit one fresh-context exec run, so an `OVERSIZE:` signal means split rather than push on. After save, runs an automatic fresh-context doc self-review via the `andthen:review` skill with `--mode doc --fix` and blocks `spec-ready` on unresolved architecture/requirements decision Notes. Supports `--visual` as a convenience handoff to the `andthen:visualize` skill for the produced FIS |
