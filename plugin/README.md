...
| `ops` | Deterministic state management, plan/FIS mutations, Tech Debt and Learnings appends, git conventions, and progress tracking. `update-fis design-change` is the audited mutation path for ADR-backed Intent/scenario amendments (missing requirements still use `discovered-requirements`); `update-ledger` (`add`/`reconcile`/`withdraw`/`bump-recurrence`/`override-close`) is the deterministic single-document mutator for the reconciliation ledger |
