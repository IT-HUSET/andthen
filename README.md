...
> "I have a feature idea" → *and then?* → **clarify** → *and then?* → **plan** → *and then?* → **spec** → *and then?* → **exec-spec** → *and then?* → **review-gap** → **ship it.**
...
Every step naturally leads to the next. *"And then?"* forces structured progression rather than ad-hoc development.

```
clarify → plan → spec → exec-spec → review-gap
                   ↑                   │
                   └── feedback loop ──┘
```
...
# Multi-feature (automated pipeline – spec → exec-spec → review per story):
