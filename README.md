...
`exec-spec` and `quick-implement` run an internal **implement → verify → evaluate** loop (build, tests, lint, and visual validation where applicable), repeating until the work holds. `review` then gives you a focused proof-led pass – code, docs, spec-vs-implementation gap, or security – with coverage evidence, attempted falsifiers, and a named code-smell baseline before you ship. Findings flow into `remediate-findings`, which applies the smallest safe fixes and re-validates.
...
**Always-loaded rules (once per machine)** – init's last step offers to wire two user-level tiers, so they load in every session and every project: the foundational rules guideline into `~/.claude/CLAUDE.md` / `~/.codex/AGENTS.md` (engineering, scope, commit and attribution rules – sub-agents see these too), and the `concise-critical` conversation style into the system prompt (Claude Code output style / Codex `developer_instructions`: terse, critical, reference codes). Answer **rules-only** to keep everything in the instruction files instead. It detects what is already wired and never overwrites your settings; to wire later, re-run `init` or use the paste-prompt in [plugin/README.md](plugin/README.md#foundational-rules-and-conversation-style).

**Manual setup** – skills read two sections from your root agent instruction file (`CLAUDE.md` / `AGENTS.md`): a **Project Document Index** (where skills write specs, plans, etc.) and **Project-Specific Guidelines**. See [`plugin/skills/init/templates/CLAUDE.template.md`](plugin/skills/init/templates/CLAUDE.template.md) for a starter, and [plugin/README.md](plugin/README.md#foundational-rules-and-conversation-style) for the manual wiring options.
...
```bash
# Just say what to review – the skill picks the lens(es):
/andthen:review "does this match the spec?" <path>        # → gap lens
/andthen:review "check correctness and security" <path>   # → code + security
/andthen:review "review PR 42"                            # → reads PR 42 (read-only)

# ...or name lenses explicitly (the equivalent flag form):
...
`review` is the default review entrypoint. It runs the right lens for the target – `code`, `doc`, `gap` (spec-vs-implementation), or `security` – inferred from what you ask for, named explicitly with `--mode`, or auto-detected from the target, with comma-separated chains consolidated into a single report. `--council` adds a multi-perspective adversarial layer – an explicit opt-in (it spends real tokens/agents), never triggered by phrasing alone. `exec-plan` already reviews each story and runs a final gap review, so this step is for manual or final checks.
...
One starter guideline ships with AndThen: `CRITICAL-RULES-AND-GUARDRAILS.md` – always-loaded engineering rules and guardrails for AI agents (scope, verification, git/commit/attribution, sub-agent routing). The canonical file lives in `plugin/skills/init/templates/guidelines/`; `docs/guidelines/` is a symlink to that directory so there's one source of truth. Its conversation-style companion – concision, critical stance, reference codes – ships as the `concise-critical` output style (`plugin/skills/init/templates/output-styles/`) for the system-prompt tier; see [Setup](#setup) for how `init` wires both at user level. All other guidelines are project-authored: add your own conventions and counter-intuitive rules, referenced from your `CLAUDE.md` / `AGENTS.md` – frontier models don't need generic best-practice guides.
