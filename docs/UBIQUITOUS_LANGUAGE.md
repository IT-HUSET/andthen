...
| Feature Implementation Specification (FIS) | Execution-sized specification generated by the `andthen:spec` skill or `andthen:plan` skill. Contains required context, success criteria, scenarios, implementation plan, testing strategy, and validation. | spec, feature spec, implementation spec |
...
| Proof-of-Work | FIS authoring rule that every success criterion needs a concrete proof path through a scenario or task Verify line. | proof note |
...
| Documentation Lookup Agent | The single plugin-tier `andthen:documentation-lookup` agent used for external documentation retrieval. | docs agent |
...
| agent | User environment | An AI coding agent such as Claude Code, Codex, Aider, or Cursor | AndThen plugin tier | The `andthen:documentation-lookup` agent only |
