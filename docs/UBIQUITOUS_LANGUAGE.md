...
| Expected Outcome | FIS-internal, behavioral, user-/business-observable success condition under `## Feature Overview and Goal`, tagged `[OC<NN>]`. Distinct from upstream PRD outcomes (referenced by Required Context) and from Structural Criteria (non-behavioral invariants). | PRD outcome, success criterion, structural criterion |
...
| Required Context | Optional FIS section for load-bearing upstream sources. Anchored references are default; bounded source-pinned inline fallbacks and legacy blocks remain authoritative. | background, references |
...
| bindingConstraints | `plan.json` top-level array of PRD anchors plus verbatim transport/fallback text. Generated FIS files reference durable anchors without narrowing the constraint. Markdown/prose section: `Binding Constraints`; singular entry: binding constraint. | hard requirements |
...
| Proof-of-Work | FIS rule that each Acceptance Scenario has an articulated contract (title plus any GWT) and concrete proof path; each Structural Criterion is proved by a task Verify line. | proof note |
...
| Worktree Isolation | Separate git worktrees for parallel execution. Review's legacy `--worktree` flag is checkout-free and does not create one. | worktree mode |
