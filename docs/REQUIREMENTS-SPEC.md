...
- `SYS-04` The shipped template's Foundational Rules section carries commented setup options only, for two tiers – engineering rules (user-level copy of the guideline preferred; @-import or path-reference lines are added by the user) and conversation style (Claude Code `outputStyle`; Codex `developer_instructions`) – no active reference line ships; the installed guideline file remains docs/guidelines/CRITICAL-RULES-AND-GUARDRAILS.md and holds every rule sub-agents must also see (engineering, git/commit/attribution, dates, artifact conventions).
...
- `SYS-56` Conversation-style tier: plugin/skills/init/templates/output-styles/concise-critical.md (registered via `outputStyles` in plugin/.claude-plugin/plugin.json) ships the conversation-only rules (critical stance, extreme concision, state-each-fact-once, plain language, named AI tells, conclusion last, introduced reference codes for 3+ items) as a Claude Code output style with `keep-coding-instructions: true` and `force-for-plugin` unset (opt-in via `outputStyle: "andthen:concise-critical"`, never forced); Codex users paste the body into `developer_instructions`. Rules governing what agents write into the repo (commits, attribution, dates, estimates, dashes) stay in CRITICAL-RULES-AND-GUARDRAILS.md because non-fork sub-agents load CLAUDE.md/AGENTS.md but not output styles; no rule is duplicated across tiers; the installer does not propagate the file separately (it travels inside the init skill), and the andthen:init skill wires it at user level on confirm (INIT-54..58).
...
- `PST-59` Regeneration contract – canonical for all writers of a derived document (today andthen:map-codebase), stated in the `## Regeneration contract` section of project-state-templates.md and cited by name from each write site: an existing document at the target path is merged, never overwritten; sections its template note marks _derived_ are regenerated from the current analysis, sections marked _judgment_ are preserved with genuinely new items appended and marked `(new)`; a row whose subject can no longer be located is flagged in place, never deleted; template headings not locatable at all (document hand-written to a different structure) means no merge is attempted – the file is left untouched, the analysis is written beside it as `<NAME>.discovered.md` under a `> Status: Discovered – requires validation by team` header, and the pair is named for reconciliation; an absent target is written fresh from the template. Writer skills keep only skill-specific gates inline.
- `PST-60` Derived/judgment classification, stated in each document's template note and referenced by PST-59: ARCHITECTURE.md derived = `Key Components`, `Integration Points`; judgment = `System Overview`, `Data Flow`, `Key Constraints`. STACK.md = every section derived, no judgment sections (an unsupported stack entry is flagged, not kept). KEY_DEVELOPMENT_COMMANDS.md = every command table derived, with hand-added commands having no discoverable source treated as judgment (flagged, never dropped). The other templates are not derived documents and PST-59 does not apply to them.
...
- `INIT-30` Final summary lists only what the current run actually created, grouped by: Core orientation stubs, starter guidelines, optional confirmed documents, user-level wiring.
...
- `INIT-33` Project file paths in the summary are printed as relative paths; user-level wiring paths are printed with `~`.
- `INIT-34` The only starter guideline file in templates/guidelines/ is CRITICAL-RULES-AND-GUARDRAILS.md; all other guidelines are project-authored (the template's Project Guidelines section is a TODO placeholder). templates/output-styles/concise-critical.md is the conversation-style companion (SYS-56), registered as a Claude Code output style via the plugin manifest's `outputStyles` field – not a guideline and never copied into the project.
- `INIT-54` Step 3 (after 2a or 2b, before the summary) offers once-per-machine user-level wiring of the always-loaded tiers. Detection per existing host directory (Claude Code: `$CLAUDE_CONFIG_DIR`, default `~/.claude/` – one config root for the instruction file, settings.json, plugins probe, and output-styles copy; Codex: `~/.codex/`): rules tier wired when the user-level instruction file contains the `# Critical Rules and Guardrails` heading and no `## Working Style` section (that section marks a pre-split copy → stale, not wired; the STOP-and-WAIT names the replacement – heading through the next top-level heading or EOF – as a missing piece); conversation tier wired when that file contains the `# Response Style` heading (earlier rules-only), or `settings.json` sets `outputStyle` to this plugin's style, or `config.toml` sets `developer_instructions`. An `outputStyle` naming another style is never changed – reported, with rules-only offered. Everything wired or no host directory → skipped silently.
- `INIT-55` Wiring choices: **both** (recommended – CRITICAL-RULES appended to the instruction file(s); the plugin's style name merged as `outputStyle` into `settings.json`; the style body added as `developer_instructions` in the top-level section of `config.toml`, created if missing), **rules-only** (CRITICAL-RULES plus the style body appended to the instruction file(s); no output style / developer_instructions – the full rule set in one tier), **skip**. Only missing pieces are offered.
- `INIT-56` Wiring is non-destructive: existing `outputStyle` / `developer_instructions` values are never edited (reported and left); settings.json is merged, never rewritten; a settings or config file that does not parse stops the step with a report; instruction files are appended, created only if missing. Style name follows the install: with the AndThen Claude Code plugin installed (listed in `<config dir>/plugins/installed_plugins.json` or cached under `<config dir>/plugins/cache/`) the plugin-namespaced `<plugin>:concise-critical`; otherwise the style file is copied to `<config dir>/output-styles/` and bare `concise-critical` is used. Shipped skill/template text never spells the namespaced name with a literal `andthen:` token – the loose-skill installer rewrites that token to the install prefix. A project-level `outputStyle` shadowing the user-level one is mentioned when present.
- `INIT-57` Re-running init on an initialized project surfaces missing user-level wiring in the Step 2b analysis and routes to Step 3 – this is the supported way to wire later.
...
- `INIT-58` Gate after Step 3: user-level wiring applied, declined, or already in place; STOP and WAIT for the both / rules-only / skip answer before touching any user-level file.
...
- `REV-04a` Skill invocation authorizes spawning required review sub-agents; a spawn tool absent from the visible tool list is not unavailability – where the host supports deferred tool loading, run tool discovery before inline fallback. Find-passes spawn as plain result-returning sub-agents: host options that make a spawn a persistent named/mailbox teammate route its output off the channel the orchestrator collects, losing the pass's findings silently. The single exception is the council Agent Teams path (REV-32), which collects each member's findings from team state; an uncollected member is an incomplete pass, never a clean one.
- `REV-04b` Every find-pass prompt carries its slice of the Step 2 coverage plan (the surfaces and falsifiers that pass owns, so Coverage Matrix rows hold the reviewing pass's own evidence rather than orchestrator back-fill) and the read-only rule, which binds every finding-producing child – find-pass or council cross-lens pass (REV-34): it never modifies the review target, and verification that would (e.g. mutating code to test test-suite strength) runs only against an isolated copy. `--fix` does not relax this; edits happen only in remediation after the report.
...
- `MAP-35` The `Stack`, `Architecture`, and `Key Dev Commands` documents are written under the Regeneration contract (PST-59, classification PST-60): an existing document at the target path is merged, not overwritten, and any `<NAME>.discovered.md` pair produced is named in the completion summary. Merge is unconditional – there is no `--update` flag, a deliberate divergence from UL-01: brownfield re-runs are this skill's normal case, not an opt-in. Documentation outputs only – `--model-only` writes none (MAP-34), and the Conventions section keeps its own append/align rule (MAP-10).
**Gates / BLOCKED**
...
- `MAP-36` Existing `ARCHITECTURE.md` / `STACK.md` / `KEY_DEVELOPMENT_COMMANDS.md` whose template headings cannot be located (hand-written to a different structure): the unlocatable-structure branch of PST-59 applies – no merge, file untouched, analysis written beside it as `<NAME>.discovered.md`, and the pair named in the completion summary for reconciliation.

**Integration**
- Reads project-state-templates.md at `${CLAUDE_PLUGIN_ROOT}/references/project-state-templates.md` for STACK.md, ARCHITECTURE.md, KEY_DEVELOPMENT_COMMANDS.md, and DECISIONS.md template shapes, and for the Regeneration contract governing writes to the first three (PST-59).
